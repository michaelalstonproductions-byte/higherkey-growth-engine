...
def build_client_delivery_manifest(config: AppConfig, *, dry_run: bool = False) -> dict[str, Any]:
    ...
    checklist = [
        ...
        _check("trial_ops", "Trial Ops", "ready" if feedback_summary.get("status") in {"ready", "needs_attention"} and issue_queue.get("status") in {"ready", "needs_attention"} else "missing", "Trial feedback inbox and issue queue are available locally." if feedback_summary else "Create the feedback template and build the issue queue after client sessions.", path=Path("out/client_delivery"), next_action="Build Issue Queue", details={"feedback_status": feedback_summary.get("status"), "issue_queue_status": issue_queue.get("status")}),
