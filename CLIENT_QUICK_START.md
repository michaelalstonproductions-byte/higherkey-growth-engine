...
11. Use Trial Ops after a client session to collect feedback, build the issue queue, create a patch plan, track patch execution, draft release notes, and create a redacted support package.
...
6. Open Trial Ops to collect feedback locally, build the trial issue queue, generate a patch plan, track patch execution, and draft client release notes for operator review.

## Trial Ops

Use **Trial Ops** to create a local feedback template, import client notes, build the issue queue, draft the patch plan, track patch execution, prepare response notes, draft release notes, and create a redacted support package. Feedback stays local by default. Response and release notes are drafts for review before sending. Support packages exclude private media, tokens, local connector config, runtime databases, and raw logs by default.
...
- **Trial Ops** to turn client feedback into a local issue queue, patch plan, patch execution board, response notes, release notes, and fix backlog.
